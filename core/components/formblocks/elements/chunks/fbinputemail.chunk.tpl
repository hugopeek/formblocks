<div class="form-group [[!+fb[[*id]].error.fb[[*id]]-email:notempty=`has-error`]]">
    ...
    <div class="[[+field_layout:is=`form-horizontal`:then=`col-sm-8`:else=`form-default`]]">
        ...
        <input type="email" class="form-control" name="fb[[*id]]-email" id="fb[[*id]]-email" value="[[!+fb[[*id]].email]]" placeholder="[[+field_placeholder]]" [[+field_helptext:notempty=`aria-describedby="fb[[*id]]-email-help"`]]>
